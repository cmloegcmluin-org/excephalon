...
- **Insulate the user from agents.** An agent's own words never reach them — not commit hashes, not
  ...
  answer — news must never die with a wedged session — and it says only what the APP knows: which piece of work, in HIS words (`desk.title_of`, his list item or the task's first line), and what happened to it. It used to relay the agent's own first sentence, which is the same failure wearing a cap: "The fresh demo is clean: exactly the four curated scenarios, two clean Excephalon messages, no raw 'Red', no 'tab' pointer" reached him whole, and every noun in it belonged to a conversation he was never part of ("what is a 'fresh' demo?? what four curated scenarios? ... basically this whole message is useless, insane, confusing, and terrible"). No agent prose, no internal name, no pointer to a log. That name is his everywhere he meets it — the roll call labels by it too, since reading out "scheduled-messages" beside his own "timed-reminder feature" made one thread read as two ("it's weird and confusing that in the previous message it chose a different name for the feature than its agent log's name"). Handed the raw stream, a person cannot tell
...
the echo both; mishearings of his own terms are the transcriber's vocabulary pass's job. `errands.py` is the quiet errand hand: small chores - move a file, tidy a folder, check one of his own services - run in one helper session, no agent tab, its outcome narrated like any other news. runtime/services.json (standard mcpServers shape, personal, gitignored) is that session's reach - Asana as a hosted MCP server OAuthed once through /mcp in an interactive `claude`; Gmail and Calendar through `google_bridge.py`, a stdio MCP SERVER over Google's CLASSIC APIs that owns its OAuth (client and tokens under runtime/google/, machine-copyable, connected once by double-clicking `Connect Google` — a .bat AND a .command, because a door built
for one desk is no door on the other: the Windows machine had only the .command, so when the
sign-in expired the only way back was a command line he does not work from, and the app told him
to use one — on the registered port 8765). It began as a forwarder to Google's hosted MCP servers and every layer was made to work except the last: with a verified-perfect sign-in - web client, their guides' exact five scopes, all four APIs enabled - every hosted tools/call still answered "The caller does not have permission", while the classic endpoints answered the same token flawlessly. Do not point anything back at the hosted servers (or the CLI's type:http transport, which also throws on their valid replies) without watching a real tools/call succeed. Errand sessions run --strict-mcp-config, because account-level claude.ai connectors attach to any session and a headless one that tries to OAuth them wedges (anthropics/claude-code#36060). The brain hears what is connected (services_note) so 'check my calendar' becomes run_errand instead of 'I can't see your calendar', and both a connected list and an unreadable file are announced at startup, because a config applied unseen - or silently not applied - reads as a broken app. Every configured service is SPOKEN TO before it is called connected (`check_services` launches each stdio server and initializes it; the Google ones also trade their refresh token, `sign_in_fault`, since this bridge starts perfectly on a sign-in Google has revoked) - announced from the config alone, a server crashing at launch on a stale path in his own file still read as reachable for a whole day, while the brain, told only the name, said the service "isn't set up yet" and had no idea how to fix it. A service that does not answer is named to him AND to the brain as set up but faulty, carrying the failure's own words (services_note's `broken`), because "not set up" is a different problem with a different fix; a bridge tool call that 401s past its own refresh answers with the connect door rather than Google's error page. Offline is never a fault: a check that cannot reach Google at all leaves the sign-in alone, since sending him to redo a good sign-in is the wrong fix confidently given. `foreman.py` is the senior layer between the talker and the workers: engaged only through the
