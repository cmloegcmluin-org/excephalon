...
def test_an_agents_log_links_back_to_the_task_it_is_working_on(tmp_path):
    ...
    # "project name, #number: task name" - the whole identifier, one link back to that exact task.
    link = section.split('<a href="/projects#task-excephalon-3"')[1].split("</a>")[0]
    clickable = re.sub(r"<[^>]+>", "", link.split(">", 1)[1]).strip()  # the link's visible text
    assert clickable == "Excephalon, #3: warn about credits"
